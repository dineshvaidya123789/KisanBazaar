...
const Buyer = () => {
    ...
    const parsedSearch = React.useMemo(() => {
        ...
        // 2. Detect Location (Basic logic: check against known districts/tehsils from locationData)
        // Since we don't have direct flat list here easily without importing large data, we rely on basic fuzzy matching or assume
        // common districts are potential matches. For optimized search, we would use a dedicated search index.
        // For now, let's treat any word that matches a known district/state in helper as location.
        // Simplified: We assume location is not passed in "smart search" often with the new UI selector.
        // Retaining basic keyword extraction for commodity.
    ...
    const filteredListings = listings.filter(item => {
        ...
        const matchesState = !selectedState || itemState.includes(selectedState.toLowerCase());
        const matchesDistrict = !selectedDistrict || itemDistrict.includes(selectedDistrict.toLowerCase());
        const matchesTehsil = !selectedTehsil || itemTehsil.includes(selectedTehsil.toLowerCase());
        ...
        return matchesCategory && matchesSearch && matchesMinPrice && matchesMaxPrice && matchesState && matchesDistrict && matchesTehsil;
    });
            ...
            {showFilters && (
                <div className="advanced-filters" style={{
                    ...
                    <div style={{ gridColumn: 'span 2' }}>
                        <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 'bold', fontSize: '0.85rem', color: '#64748b' }}>📍 Location (स्थान):</label>
                        <LocationSelector
