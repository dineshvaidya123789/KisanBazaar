...
const getFallbackWeather = (city) => {
    ...
    for (const [state, districts] of Object.entries(locationData)) {
        // Check if city matches a district
        if (districts[city]) {
            foundState = state;
            break;
        }
        // Check if city matches a tehsil
        for (const districtData of Object.values(districts)) {
            if (districtData.tehsils && districtData.tehsils.includes(city)) {
                foundState = state;
                break;
            }
        }
    ...
    // Default to MP if still unknown, or keep relevant default
